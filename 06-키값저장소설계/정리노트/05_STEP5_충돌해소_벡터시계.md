...
> **전제 — 불변 버전(immutable version):** 데이터를 바꿀 때 기존 값을 덮어쓰는 게 아니라 **새 버전을 만든다.** 각 버전은 변경 불가능하므로, 충돌이 나도 두 버전이 **모두 보존**되어 사후에 합칠 수 있다.
...
| `[서버:카운터]` 목록이 길어짐 | 갱신 서버가 많아질수록 길이 증가 → **길이 제한(임계치 초과 시 오래된 쌍 제거)** 으로 완화. 단, 제거하면 선후 관계가 **부정확**해질 수 있음 |

> 다만 **아마존 Dynamo 문헌에 따르면, 실제 서비스에서 이 목록 폭증 문제가 벌어진 적은 없다**고 한다. → 대부분의 서비스에서 벡터 시계는 충분히 실용적.

---

## 6. 실무 선택 — 그래서 기본은 LWW

벡터 시계는 "정확하지만 비싸다"(클라이언트 복잡도 + 메타데이터 증가). 그래서 시스템마다 선택이 갈린다.

| 시스템 | 충돌 해소 | 비고 |
|------|---------|------|
| **Dynamo · Riak** | **벡터 시계** | 동시 쓰기를 보존, 클라이언트가 머지 (장바구니 등) |
| **Cassandra** | **LWW** (타임스탬프 최신값 승리) | 단순·저오버헤드, 동시 쓰기 일부 유실 감수 |

> **이 설계(Cassandra 스타일)의 기본값은 LWW.** 주 사용처(캐시·세션·프로필)는 동시 쓰기 충돌이 드물어 단순함의 이점이 더 크다. **동시 쓰기 손실을 반드시 추적해야 하는 워크로드(장바구니 병합 등)에 한해 벡터 시계를 선택적으로 활성화**한다.
> ⚠️ 주의: LWW는 §1에서 본 **시계 어긋남(clock skew)** 문제를 그대로 안고 가므로, NTP 동기화 품질에 의존한다.
...
- [ ] 벡터 시계의 단점 2가지를 안다
- [ ] Dynamo/Riak(벡터 시계) vs Cassandra(LWW)의 선택 차이와, 이 설계가 LWW를 기본으로 두는 이유를 안다
...
**Q5. 벡터 시계의 단점과 완화책은?**
> ① 클라이언트가 충돌 해소 로직을 구현해야 해 복잡도가 올라간다. ② 갱신에 참여한 서버가 많아질수록 `[서버:카운터]` 목록이 길어진다 → **길이 임계치를 두고 오래된 쌍을 제거**해 완화한다(단 선후 관계가 부정확해질 수 있음). 다만 아마존은 실무에서 이 문제를 겪은 적이 없다고 한다.

**Q6. 벡터 시계가 정확한데도 Cassandra나 이 설계는 왜 LWW를 기본으로 쓰나요?**
> 벡터 시계는 정확하지만 **클라이언트 복잡도와 메타데이터 증가**라는 비용이 있다. 캐시·세션·프로필처럼 동시 쓰기 충돌이 드문 사용처에서는 **LWW(타임스탬프 최신값 승리)** 의 단순함·낮은 오버헤드가 더 유리하다. 동시 쓰기 손실을 반드시 추적해야 하는 경우(장바구니 병합 등)에만 벡터 시계를 선택적으로 켠다. 대신 LWW는 **시계 어긋남**에 의존하므로 NTP 품질이 전제된다.
